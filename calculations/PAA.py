...
class PAA():
    ...
    def PAA_func(self):
        ...
        for i in range(1,4):
            ...
            u_l_calc=[]
            u_r_calc=[]
            [[v_l_func,v_r_func,u_l_func,u_r_func],[dt_l_tot_vec,dt_r_tot_vec],[L_sl_func,L_sr_func,L_rl_func,L_rr_func]] = send_func(Orbit,i,delay=self.delay,LISA=LISA,arm_influence=arm_influence)
        ...
        for i in range(1,4):        
            ...
            for j in range(0,len(t_calc[i])):     
                ...
                if check_good == True:
                    ...
                    #ang_sr_l_vec.append(np.cross(v_l_calc,-u_l_calc)/)LA.norm(v_l_calc)*LA.norm(-u_l_calc),dot=True))
        ...
        if plot_on==True:
            ...
            for i in range(0,len(ang_sc)):
                ...
                w5.append(ang_sc[i][minp:maxp] - ang_beam[i][minp:maxp])
            ...
            for i in range(0,len(v_inplane)):
                ...
                w1.append(x*day2sec)
                w1.append(v_inplane[i])
                ax[1].plot(x,v_outplane[i],label='SC'+str(i+1))
                w2.append(x*day2sec)
                w2.append(v_outplane[i])
            ...
            def write_out(folder=dir_savefig,filename='',orbit_name='',variable_dict=self.write_info,mark=';   '):

                if orbit_name=='':
                    orbit_name = filename_save
        ...
        return [[lisa_cache,lisa_orb,Orbit],[v_l,v_r,u_l,u_r],PAA_ret,other_ret]
#
