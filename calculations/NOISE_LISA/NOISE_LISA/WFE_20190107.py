...
# WFE receive
    ...
    def WFE_rec(self,zmn,thmn,xlist=False,ylist=False,fast_cal=True,z0=0):
        if xlist==False:
            xlist = self.xlist
        if ylist == False:
            ylist =self.ylist
        ...
        #z0 = piston
        z0 = 0 #...adjust, when equal to piston, large number (subtract zoff)
        for i in range(0,len(xlist)):
            for j in range(0,len(ylist)):
                x = xlist[i]
                y = ylist[j]
                z = z0 #...adjust, when equal to piston, large number (subtract zoff)
        ...
        w_rec = self.w(z0)
        C = 1.0/(np.pi*(w_rec**2))*dksi
        a0=a0*C
        a1 = a1*C
        a2 = a2*-0.5*C
        a3 = a3*(-1.0/6.0)*C
        
        u = np.pi*(w_rec**2)*(a0+1j*a1+a2+1j*a3)

        return u,[a0,a1,a2,a3]
